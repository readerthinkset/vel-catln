"""
YouTube Upload Script - VELOCITY CATALAN
"""
import os
...
def generate_video_metadata(category, num_phrases, phrases=None):
    title = f"Catalan Learning: {num_phrases} Essential {category} Phrases"
    lines = [f"Learn Catalan with VELOCITY CATALAN!", "", f"Category: {category}", "", f"Master Catalan one phrase at a time! Today's {category} lesson:", ""]
    if phrases:
        for i, p in enumerate(phrases[:5], 0):
            lines.append(f"{i+1}. {p['english']}")
            lines.append(f"   {p.get('catalan', '')}")
            lines.append(f"   [{p.get('transliteration', '')}]")
            lines.append("")
    lines.extend(["Tip: Repeat each phrase out loud 3 times!", "Like this video if you learned something new!", "Subscribe for daily catalan lessons!", "", f"#LearnCatalan #CatalanLessons #CatalanForBeginners #LanguageLearning", f"#Catalan #Education #Tutorial #DailyCatalan #{category.replace(' ', '')}", f"#VELOCITYCATALAN #CatalanPhrases #SpeakCatalan"])
    tags = [f"learn catalan", f"catalan lessons", f"catalan for beginners", f"catalan phrases", "language learning", f"speak catalan", category.lower(), "education", f"daily catalan", "velocity catalan", f"catalan learning"]
    return title, "\n".join(lines), tags
...
def main():
    ...
    try: upload_to_youtube(video_path=video_file, title=f"Learn Catalan Daily", description=f"#shorts #learncatalan #catalan #language #education", tags=[hashtag, 'education', 'language learning'], category_id='22')
